...
class InitialInfo extends Component {
  ...
  handleChange = (name, value) => {
    ...
    const { desiredWeight } = this.state;

    switch (name) {
      case "height":
      height = valueToNumber > 10 && valueToNumber <= 300;
        break;

      case "age":
      age = valueToNumber > 18 && valueToNumber <= 99;
        break;

      case "currentWeight":
      currentWeight = valueToNumber > 10 && valueToNumber <= 300;
        break;

      case "desiredWeight":
      desiredWeight = valueToNumber > 10 && valueToNumber <= 300;
        break;
  ...
  validationForm = () => {
    ...
    this.setState({
      isValid: height && age && currentWeight && desiredWeight
    });
  };
    ...
    return (
      <ScrollView>
        <View style={headerDesign}>
          ...
          <TouchableOpacity
            onPress={() => {
              this.props.navigation.jumpTo("Regist");
            }}
            style={{ position: "absolute", top: 27, left: 10, padding: 10 }}
          >
